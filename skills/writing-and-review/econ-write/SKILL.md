---
name: econ-write
description: Draft and revise economics, finance, and accounting papers using Cochrane/McCloskey/Shapiro-style craft: triangular introductions, concrete magnitudes, identification-first structure. Use with econ-humanizer and econ-writing-plus.
curated-by: John Barrios
collection: barrios-skills
upstream: https://github.com/hanlulong/econ-writing-skill
license: MIT
---
...
- **Use this** for first drafts, section rewrites, and structure.
- Then run **`econ-humanizer` / `econ-humanizer-plus`** to strip AI tells.
- Use **`econ-writing-plus`** for method-specific ID language (modern DiD, IV, RDD, accounting archival designs).
- For accounting/finance venues (JAR, TAR, JAE, JF, JFE, RFS), state magnitudes in economic terms (bps, ROA pp, abnormal returns) and name the identification threat that matters for that literature.

Adapted from OpenEcon [`econ-writing-skill`](https://github.com/hanlulong/econ-writing-skill) (MIT, Lu Han).
